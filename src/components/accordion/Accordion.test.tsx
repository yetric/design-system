...
describe("Accordion", () => {
  ...
  it("content is hidden by default", () => {
    ...
    expect(screen.getByRole("button", { name: /Question 1/i })).toHaveAttribute("aria-expanded", "false");
  });

  it("opens an item when trigger is clicked", async () => {
    render(<BasicAccordion />);
    await userEvent.click(screen.getByText("Question 1"));
    expect(screen.getByText("Answer 1").closest("[data-state]")).toHaveAttribute("data-state", "open");
  });
  ...
  it("multiple type opens several items simultaneously", async () => {
    render(
      <Accordion type="multiple">
        <AccordionItem value="a"><AccordionTrigger>A</AccordionTrigger><AccordionContent>CA</AccordionContent></AccordionItem>
        <AccordionItem value="b"><AccordionTrigger>B</AccordionTrigger><AccordionContent>CB</AccordionContent></AccordionItem>
      </Accordion>
    );
