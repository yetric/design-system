...
const Cell = ({ label }: { label: string }) => (
  <Box p="sm" radius="md" style={{ background: "hsl(var(--muted))", textAlign: "center" }}>
    <Text size="body-sm" color="muted">{label}</Text>
  </Box>
);
...
export const Responsive: Story = {
  ...
  render: () => (
    <Grid cols={{ base: 1, md: 2, lg: 4 }} gap={4}>
      {Array.from({ length: 8 }, (_, i) => <Cell key={i} label={`Item ${i + 1}`} />)}
    </Grid>
  ),
};
...
export const TwoColumns: Story = {
  ...
  render: (args) => (
    <Grid {...args}>
      {Array.from({ length: 4 }, (_, i) => <Cell key={i} label={`Item ${i + 1}`} />)}
    </Grid>
  ),
};
...
export const ThreeColumns: Story = {
  ...
  render: (args) => (
    <Grid {...args}>
      {Array.from({ length: 6 }, (_, i) => <Cell key={i} label={`Item ${i + 1}`} />)}
    </Grid>
  ),
};
  ...
  render: () => (
    <Grid cols={3} gap={4} align="start">
      <Box p="sm" radius="md" style={{ background: "hsl(var(--muted))", height: 80 }}>
        <Text size="body-sm" color="muted">Tall</Text>
      </Box>
      <Cell label="Short" />
      <Cell label="Short" />
    </Grid>
...
export const Playground: Story = {
  ...
  render: (args) => (
    <Grid {...args}>
      {Array.from({ length: typeof args.cols === "number" ? args.cols : 3 }, (_, i) => <Cell key={i} label={`Item ${i + 1}`} />)}
    </Grid>
  ),
};
