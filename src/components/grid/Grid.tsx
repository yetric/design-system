...
const Grid = React.forwardRef<HTMLElement, GridProps>(
  (
    { cols = 1, gap = 4, gapX, gapY, align, justify, as: Comp = "div", style, className, ...props },
    ref
  ) => {
      ...
      <>
        ...
        <Comp
          ...
          style={{
            display: "grid",
            gridTemplateColumns:
              typeof cols === "number" ? `repeat(${cols}, minmax(0, 1fr))` : undefined,
            columnGap: gapX !== undefined ? `${gapX * 0.25}rem` : undefined,
            rowGap: gapY !== undefined ? `${gapY * 0.25}rem` : undefined,
            gap: gapX === undefined && gapY === undefined ? `${gap * 0.25}rem` : undefined,
            alignItems: align,
