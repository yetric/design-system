...
export const Default: Story = {
  render: () => (
    <Table>
      ...
      <TableHeader><InvoiceHeaders /></TableHeader>
  ...
  render: () => (
    <div className="flex flex-col gap-8">
      {(["compact", "default", "comfortable"] as const).map((d) => (
        <div key={d}>
          <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">{d}</p>
          <Table density={d}>
            <TableHeader><InvoiceHeaders /></TableHeader>
            <TableBody><InvoiceRows /></TableBody>
          </Table>
  ...
  render: () => (
    <div className="flex flex-col gap-8">
      {(["rows", "grid", "outer", "none"] as const).map((b) => (
        <div key={b}>
          <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">{b}</p>
          <Table borders={b}>
            <TableHeader><InvoiceHeaders /></TableHeader>
            <TableBody><InvoiceRows /></TableBody>
          </Table>
